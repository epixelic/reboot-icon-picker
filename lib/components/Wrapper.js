...
class Wrapper extends React.Component {
    ...
    getFilteredIcons(){
        ...
        return this.state.icons.filter(icon => {
            ...
            }).length >= 1 || icon.label.toLowerCase().includes(trimmedSearchState) || (!trimmedSearchState.includes('fa-') && similarity(icon.label, trimmedSearchState) >= 0.7) || (icon.aliases && icon.aliases.includes(trimmedSearchState));
        });
    }
