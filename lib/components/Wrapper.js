...
class Wrapper extends React.Component {
    ...
    refreshIconPickerInputs(){
        // refresh existing inputs
        let iconPickerInputs = Array.from(document.querySelectorAll(this.props.selector));
        iconPickerInputs.forEach(iconPickerInput => {
            let dropdownButton =  {
                iconBtn: iconPickerInput.parentNode.querySelector('.dropdown-button'),
                unselectBtn: iconPickerInput.parentNode.querySelector('.unselect-button'),
                btnGrp: iconPickerInput.parentNode.querySelector('.btn-group')
            };
            this.setSelectedIconWithInputValue(iconPickerInput, dropdownButton);
        });
            ...
            try{
                ...
                if(decodedValue.pack && decodedValue.icon  && this.state.icons){
                    let selectedIcon = this.state.icons.find((icon) => {
                        return (icon.class == decodedValue.icon || (icon.aliases && icon.aliases.includes(decodedValue.icon))) && icon.packSlug == decodedValue.pack
                    });
    ...
    getFilteredIcons(){
        ...
        // filter only terms
        if(this.state.onlySearchInTerms){
            return this.state.icons.filter(icon => {
                return icon.terms.filter(term => {
                    return term.toLowerCase() === trimmedSearchState;
                }).length >= 1;
            });
        }

        return this.state.icons.filter(icon => {
            if(trimmedSearchState.startsWith('fa-')){
                return icon.class === trimmedSearchState;
            }else {
                return icon.terms.filter(term => {
                    return term.toLowerCase().includes(trimmedSearchState) || similarity(term, trimmedSearchState) >= 0.7;
                }).length >= 1 || icon.label.toLowerCase().includes(trimmedSearchState) || similarity(icon.label, trimmedSearchState) >= 0.7 || icon.class.toLowerCase().includes(trimmedSearchState) || similarity(icon.class, trimmedSearchState) >= 0.7 || (icon.aliases && icon.aliases.includes(trimmedSearchState));
            }
        });
    }
    ...
    onIconSelect(icon){
        ...
        this.state.inputTarget.dispatchEvent(new Event('change'));
    }
