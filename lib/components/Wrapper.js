...
class Wrapper extends React.Component {
        ...
        iconPickerInputs.forEach(iconPickerInput => {
            ...
            dropdownButton.iconBtn.addEventListener('click', () => {
                ...
                if(!result.iconSelected){
                    this.setState({
                        search: `terms:${result.inputIconClass}`
                    });
                }
            });
    ...
    getFilteredIcons(){
        ...
        // filter only terms
        if(trimmedSearchState.startsWith('terms:')){
            trimmedSearchState = trimmedSearchState.substring(6);
        ...
        return this.state.icons.filter(icon => {
            if(trimmedSearchState.startsWith('fa-')){
                ...
                }).length >= 1 || icon.label.toLowerCase().includes(trimmedSearchState) || similarity(icon.label, trimmedSearchState) >= 0.7 || (icon.aliases && icon.aliases.includes(trimmedSearchState));
            }
        });
    }
